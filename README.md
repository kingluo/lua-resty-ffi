...
In Go, it means the output library generated by `go build` e.g. `lib_ffi_go_etcd.so`.

In Python3, it means the shim library `lib_ffi_python3.so`, and native python3 modules.

In Java, it means the shim library `lib_ffi_java.so`, and native Java classes/jar.
...
local demo = ngx.load_ffi("lib_ffi_python3.so",
...
patch -p1 < /opt/lua-resty-ffi/patches/config.patch
cd /opt/openresty-1.21.4.1/bundle/lua-resty-core-0.1.23/lib/resty/
patch -p1 < /opt/lua-resty-ffi/patches/core.lua.patch
cd /opt/openresty-1.21.4.1/bundle/nginx-1.21.4/src/core/
patch -p1 < /opt/lua-resty-ffi/patches/ngx_thread_pool.c.patch

cd /opt/lua-resty-ffi/
cp -a ngx_http_lua_ffi.c /opt/openresty-1.21.4.1/bundle/ngx_lua-0.10.21/src/
cp -a ffi.lua /opt/openresty-1.21.4.1/bundle/lua-resty-core-0.1.23/lib/resty/core/
